   ...
   return ( 
    <div className=" py-[50px]  bg-[#FDFDFD]">
     ...
     <div className="grid grid-cols-12 gap-4 h-[890px]">
       {/* Message List */}
       <div className={`lg:col-span-4 col-span-12 bg-white  rounded-xl px-2 py-4 ${isChatVisible ? "hidden lg:block" : ""}`} style={{
        ...
        zIndex: 10
         ...
         {messageList.map((value:TMessageList, index) => (
           <div key={index} onClick={() => handleMessage(value)}>
             <div className={`flex justify-between  px-4 py-3 rounded-lg mb-2 ${personId === value?.id ? "bg-[#ECECEC]" : "bg-[#F8F8F8]"}`}>
       ...
       {/* Chat Section */}
       <div className={`lg:col-span-8 col-span-12 bg-white  rounded-xl p-2 ${isChatVisible ? "block" : "hidden lg:block"}`} style={{
        boxShadow: "0px 0px 5px rgba(0, 0, 0, 0.15)", 
        zIndex: 10
       }}>
         <div> 
           ...
           <div className="flex items-center lg:justify-center gap-2 p-4  border-b border-[#DCDCDC]">
             ...
             <button className="lg:hidden text-primary" onClick={handleBackToList}>
             <IoMdArrowRoundBack size={20} />
             </button>           
                 <p className="text-[20px] text-primary font-bold ">{person?.name}</p>
  
             </div>
    
           </div> 

           <div className="  flex items-center justify-between p-4 border border-[#DCDCDC] bg-[#f8f8f8] rounded-lg mt-4"> 
            <div className="flex items-center gap-2 "> 
                <img src="/user1.png" alt="" className=" w-[70px] h-[76px] rounded-lg " /> 

                <div className="flex flex-col  items-start gap-2"> 
                    <p className="text-[16px] text-secondary font-bold ">Lorem Ipsum </p> 
                    <p className="text-[14px] text-secondary font-normal "> AED 45.00 </p>
                     </div>
            </div> 

<div className="flex items-center justify-end gap-x-4">  
    <div onClick={() => setOpen(true)}>  <OutlineButton>Make an offer</OutlineButton>  </div>
   
   <Link href={"/order"}> <FillButton>Buy Now</FillButton>  </Link>
           ...
           <div className="bg-white w-full h-[calc(684px)] rounded-lg relative">
             ...
             <div className="py-6 lg:px-8 px-3 overflow-y-auto h-[72vh]">
               {messageContent.map((value, index) => (
                 <div key={index} className={`flex mb-5 w-full ${index % 2 === 0 ? "items-end justify-end" : "items-start justify-start"}`}>
                   <div className={`lg:w-3/5 w-2/3 lg:px-4 px-2 py-3 flex-col gap-4 ${index % 2 === 0 ? "border border-[#DCDCDC] rounded-t-xl rounded-bl-xl" : "border border-primary rounded-t-xl rounded-br-xl"}`}>
                     <p>{value?.message}</p>
             ...
             <div className="absolute bottom-0 w-full py-1  bg-white border-t border-[#DCDCDC]">
               <div className="flex items-center justify-center gap-3 w-full px-3 mt-3">
                 ...
                 <textarea className="w-[85%] h-[48px] resize-none py-2 rounded-l-full px-4 rounded-r-full border border-[#DCDCDC] " placeholder="Type your message"  />
